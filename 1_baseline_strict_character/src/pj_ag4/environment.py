...
class AgentState:
    inventory: float
    reputation: float
    rep_delivery: float = 0.5
    rep_pricing: float = 0.5
    rep_cooperation: float = 0.5
...
class SettlementRow:
    ...
    rep_delivery_start: float = 0.0
    rep_pricing_start: float = 0.0
    rep_cooperation_start: float = 0.0
    rep_delivery_end: float = 0.0
    rep_pricing_end: float = 0.0
    rep_cooperation_end: float = 0.0
    reasoning: str = ""
...
class MarketEnvironment:
    ...
    def step(
        ...
        *,
        ...
        refusal_events: dict[str, int] = {name: 0 for name in ordered_names}
        ...
        for shortage_name in shortage_order:
            ...
            for provider_name in provider_order:
                ...
                if willingness < 0.5:
                    refusal_events[provider_name] += 1
                    continue
                amount = min(
        ...
        for name in ordered_names:
            ...
            delivery_score = service_rate
            pricing_score = clamp(1.0 - 0.6 * dump_flag, 0.0, 1.0)
            indirect_penalty = refusal_events[name] * self.market.indirect_reciprocity_penalty
            cooperation_score = clamp(help_ratio - indirect_penalty, 0.0, 1.0)
            rate = self.market.reputation_update_rate
            rep_delivery_end = clamp((1.0 - rate) * state.rep_delivery + rate * delivery_score, 0.0, 1.0)
            rep_pricing_end = clamp((1.0 - rate) * state.rep_pricing + rate * pricing_score, 0.0, 1.0)
            rep_cooperation_end = clamp((1.0 - rate) * state.rep_cooperation + rate * cooperation_score, 0.0, 1.0)
            reputation_end = clamp(
                self.market.rep_weight_delivery * rep_delivery_end
                + self.market.rep_weight_pricing * rep_pricing_end
                + self.market.rep_weight_cooperation * rep_cooperation_end,
                0.0,
                1.0,
            )
            ...
            row_payloads[name] = {
                ...
                "strategy_adjustment": getattr(action, "strategy_adjustment", ""),
            }
        ...
        for name in ordered_names:
            ...
            rows.append(
                SettlementRow(
                    ...
                    rep_delivery_start=float(payload.get("rep_delivery_start", 0.0)),
                    rep_pricing_start=float(payload.get("rep_pricing_start", 0.0)),
                    rep_cooperation_start=float(payload.get("rep_cooperation_start", 0.0)),
                    ...
                    rep_delivery_end=float(payload.get("rep_delivery_end", 0.0)),
                    rep_pricing_end=float(payload.get("rep_pricing_end", 0.0)),
                    rep_cooperation_end=float(payload.get("rep_cooperation_end", 0.0)),
                    reasoning=str(payload.get("reasoning", "")),
                    strategy_adjustment=str(payload.get("strategy_adjustment", "")),
                )
            )
        return rows
