...
class RiskGateStage:
    ...
    def review(
        ...
        *,
        ...
        # Universal quantity cap: at the chosen price, we must not be guaranteed to lose money
        # Cap quantity so that avg cost doesn't exceed price ceiling minus a small margin
        max_economic_qty = int((self._config.price_ceiling - self._config.linear_cost - 0.2) / max(self._config.quadratic_cost, 0.001))
        reviewed_quantity = min(reviewed_quantity, max_economic_qty)
...
class LLMPolicyAgent(RolePipelineAgent):
    ...
    def _estimate_profit(self, observation: MarketObservation, action: AgentAction, *, conservative: bool = True) -> float:
        """Estimate expected profit. If conservative=True, assume only 40% of forecast converts to sales."""
        total_supply = observation.own_inventory + action.quantity
        sales_proxy = action.forecast_demand * 0.4 if conservative else action.forecast_demand
        expected_sales = min(total_supply, sales_proxy)
        shortage = max(0.0, sales_proxy - expected_sales)
        inventory_end = max(0.0, total_supply - expected_sales)
        revenue = expected_sales * action.price
        prod_cost = action.quantity * self.config.linear_cost + (action.quantity ** 2) * self.config.quadratic_cost
        holding_cost = inventory_end * self.config.holding_cost_rate
        obsolescence_cost = inventory_end * self.config.obsolescence_penalty
        sla_penalty = shortage * self.config.sla_penalty
        menu_cost = abs(action.price - observation.own_last_price) * self.config.menu_cost_rate
        return revenue - prod_cost - holding_cost - obsolescence_cost - sla_penalty - menu_cost

    def _clamp_quantity_for_profit(self, observation: MarketObservation, price: float, forecast: int, quantity: int) -> int:
        """Reduce quantity if current level is unprofitable under conservative sales assumption."""
        for q in range(quantity, 0, -self.config.quantity_step):
            test_action = AgentAction(forecast_demand=forecast, price=price, quantity=q)
            if self._estimate_profit(observation, test_action, conservative=True) > 0:
                return q
        return self.config.quantity_step
    ...
    def decide(self, observation: MarketObservation) -> AgentAction:
        ...
        try:
            ...
            # 3. Profit safety: if unprofitable under conservative estimate, lower quantity
            test_action = AgentAction(forecast_demand=forecast, price=price, quantity=quantity)
            if self._estimate_profit(observation, test_action, conservative=True) <= 0:
                new_qty = self._clamp_quantity_for_profit(observation, price, forecast, quantity)
                if new_qty < quantity:
                    old_qty = quantity
                    quantity = new_qty
                    reasoning += f" [Profit safety: cut quantity from {old_qty} to {new_qty} to avoid loss]"
                else:
                    # Even at minimum quantity we can't profit with this price; try raising price
                    total_supply = observation.own_inventory + quantity
                    expected_sales = min(total_supply, forecast * 0.4)
                    if expected_sales > 0:
                        inventory_end = max(0.0, total_supply - expected_sales)
                        shortage = max(0.0, forecast * 0.4 - expected_sales)
                        prod_cost = quantity * self.config.linear_cost + (quantity ** 2) * self.config.quadratic_cost
                        other_costs = (
                            inventory_end * (self.config.holding_cost_rate + self.config.obsolescence_penalty)
                            + shortage * self.config.sla_penalty
                            + abs(price - observation.own_last_price) * self.config.menu_cost_rate
                        )
                        total_cost = prod_cost + other_costs
                        min_profitable_price = total_cost / expected_sales + self.config.price_step
                        new_price = max(price, min_profitable_price)
                        new_price = round_to_step(
                            new_price,
                            self.config.price_step,
                            self.config.price_floor,
                            self.config.price_ceiling,
                        )
                        if new_price <= self.config.price_ceiling:
                            new_revenue = expected_sales * new_price
                            if new_revenue > total_cost:
                                reasoning += f" [Profit safety: raised price from {price:.2f} to {new_price:.2f}]"
                                price = new_price
                            else:
                                return fallback
                        else:
