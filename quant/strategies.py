...
from pj_ag4.agents import HeuristicAgent, build_agents  # noqa: E402
...
def strategy_title(strategy_name: str) -> str:
    return {
        STRATEGY_HEURISTIC: "Heuristic",
        STRATEGY_LLM: "LLM",
        STRATEGY_RULE_PRICE_CUTTER: "Rule Price Cutter",
        STRATEGY_RULE_INVENTORY_GUARD: "Rule Inventory Guard",
...
def build_strategy_agents(
    ...
    *,
    ...
    if strategy_name == STRATEGY_HEURISTIC:
        return build_agents(config.agents, mode=STRATEGY_HEURISTIC, llm_config=llm_config)
    if strategy_name == STRATEGY_LLM:
        if llm_config is None:
            raise ValueError("llm_config is required for llm strategy")
        return build_agents(config.agents, mode=STRATEGY_LLM, llm_config=llm_config)
    if strategy_name == STRATEGY_RULE_PRICE_CUTTER:
        return _build_rule_agents(tuple(config.agents), RulePriceCutterAgent)
    if strategy_name == STRATEGY_RULE_INVENTORY_GUARD:
        return _build_rule_agents(tuple(config.agents), RuleInventoryGuardAgent)
